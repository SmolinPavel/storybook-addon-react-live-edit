...
> {
  ...
  componentDidUpdate() {
    const preparedTheme =
      typeof this.state.theme === "string" ? this.state.theme : "ctm";
    if (this.state.code !== null && this.props.active) {
      const theme =
        themeNameToVarName(preparedTheme) in themes ? preparedTheme : "default";
      if (!this.codeMirror) {
        this.codeMirror = CodeMirror(this.codeMirrorRef, {
          mode: "jsx",
          theme
        });
  ...
  render(): any {
    ...
    const themeVarName = 'default'; // themeNameToVarName(this.state.theme);
    return (
      <div
        key={themeVarName}
        style={{
      ...
      >
        {themeVarName in themes && (
          <style dangerouslySetInnerHTML={{ __html: themes[themeVarName] }} />
        )}
